...
const vectorNorm = (V: Point2): number => Math.sqrt(V.x * V.x + V.y * V.y);
...
const vectorAdd = (V1: Point2, V2: Point2): Point2 => ({
  x: V1.x + V2.x,
  y: V1.y + V2.y,
});
const vectorSub = (V1: Point2, V2: Point2): Point2 => ({
  x: V1.x - V2.x,
  y: V1.y - V2.y,
});
const vectorMult = (V: Point2, alpha: number): Point2 => ({
  x: alpha * V.x,
  y: alpha * V.y,
});
