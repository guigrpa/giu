...
const Contents = ({ id }) => {
  ...
  switch (id) {
    ...
    case 'demo:validation-intro2': el = <ValidationIntro2 />; break;
    ...
    case 'demo:drop-down-menus': el = <DropDownMenus />; break;
    case 'demo:modals': el = <ModalDemo />; break;
    case 'demo:notifications': el = <Buttons />; break;
    case 'demo:hints': el = <HintDemo />; break;
    case 'demo:buttons': el = <Buttons fIncludeDisabled />; break;
    case 'demo:icons': el = <Icons />; break;
    case 'demo:large-messages': el = <LargeMessages />; break;
    case 'demo:progress': el = <ProgressDemo />; break;
    case 'demo:hoverable': el = <HoverableDemo />; break;
...
const ValidationIntro = () =>
  <Centered>
    <DateInput />
    <div><i>
      (just focus, enter a valid/invalid date and
...
const ValidationIntro2 = () =>
  <CenteredFlex>
    ...
    <NumberInput placeholder="5 <= x <= 10" validators={[isGte(5), isLte(10)]} />
  </CenteredFlex>;
  ...
  render() {
    ...
    return (
      <div>
        ...
        <p><b>Variants:</b> DateInputs can be of three types:</p>
...
class DropDownMenus extends React.Component {
  render() {
    return (
      <Centered>
        <DropDownMenu
          items={getExampleItems({ fSeparator: true, fKeys: true, modifier: 'mod' })}
          onClickItem={(ev, fruits) => createNotif({
            title: 'Got it!',
...
class ModalDemo extends React.Component {
  ...
  addModal() {
    ...
    const children = (
      <div>
        <CenteredFlex>
          <TextInput ref={c => { this.refName = c; }}
            autoFocus
            required errorZ={52}
          />
          <DateInput
        ...
        <div><i>Try typing an invalid date...</i></div>
      </div>
    );
    modalPush({
      title,
      children,
      buttons: [
        { label: 'Hello!', onClick: () => this.addModal2(), defaultButton: true },
...
class LargeMessages extends React.Component {
  ...
  componentDidMount() {
    setInterval(() => { this.setState({ value: this.state.value + 1 }); }, 4000);
  }
  render() {
    let el;
